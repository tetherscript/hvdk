...
begin
  ...
  if HidDev.ProductName = HIDDEV_PRODUCTNAME_MOUSEABS then
  begin
    Log('Found ' + HIDDEV_PRODUCTNAME_MOUSEABS + ' driver.');
...
begin
  if Assigned(FCurrentDeviceMouse) then
  begin
    Log('Sending data to mouse driver...');
    SendMouseAbs;
...
procedure TfrmMain.SendMouseAbs;
var
Feature: TSetFeatureMouseAbs;
sX, sY, sBtn: string;
btnval: Byte;
begin
  ...
  begin
    ...
    Feature.X := Word(spnX.Value);
    Feature.Y := Word(spnY.Value);;
    //calc button value
    btnval := 0;
